...
![image-20210129105343000](image/image-20210129105343000.png)
...
![image-20210129105428750](image/image-20210129105428750.png)
...
public class Demo1 {
    public static void main(String[] args) {
        System.out.println("这里是main线程");
        Thread mt = new Thread(new MyThread(),"小强");
        mt.start();//开启了一个新的线程
        for (int i = 0; i < 20; i++) {
            System.err.println("旺财:"+i);
        }
...
![image-20210129111632966](image/image-20210129111632966.png)

------

![image-20210129111857961](image/image-20210129111857961.png)
...
![image-20210129115032597](image/image-20210129115032597.png)
...
![image-20210129115105730](image/image-20210129115105730.png)
    ...
    public static void main(String[] args) {
        new Thread(()->{
            while (true){
                synchronized (obj){
                    try {
                        System.out.println(Thread.currentThread().getName()+"------获取线程锁，调用wait等待");
                        obj.wait();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    System.out.println(Thread.currentThread().getName()+"------从wait中醒来，获取线程锁，继续执行");
                }
            }
        },"等待进程").start();
        new Thread(()->{
            while (true){
                synchronized (obj){
                    try {
                        System.out.println(Thread.currentThread().getName()+"------获取线程锁，等待3秒");
                        Thread.sleep(3000);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    System.out.println(Thread.currentThread().getName()+"------获取线程锁，调用notify，释放锁对象");
                    obj.notify();
                }
            }
        },"唤醒进程").start();
...
当多个线程协作时，比如A，B线程，如果A线程在Runnable（可运行）状态中调用了wait()方法那么A线程就进入了Waiting（无限等待）状态，同时失去了同步锁。假如这个时候B线程获取到了同步锁，在运行状态中调用了notify()方法，那么就会将无限等待的A线程唤醒。

注意是唤醒，如果获取到锁对象，那么A线程唤醒后就进入Runnable（可运行）状态；如果没有获取锁对象，那么就进入到Blocked（锁阻塞状态）。

Waiting 线程状态图

![image-20210129121032836](image/image-20210129121032836.png)

## 3.5 补充知识点

到此为止我们已经对线程状态有了基本的认识，想要有更多的了解，详情可以见下图：

![image-20210129123124185](image/image-20210129123124185.png)
