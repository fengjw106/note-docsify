...
![image-20210226152018776](image/image-20210226152018776.png)
...
class DemoApplicationTests {
    ...
    @Test
    void contextLoads() {
        Integer integer = jdbcTemplate.queryForObject("select count(*) from user where id", Integer.class);
        log.info("查询的个数："+integer);
    }
...
public class DruidDataSourceConfig {
    ...
    @Bean
//    默认的自动配置是判断容器里没有才会配 @ConditionalOnMissingBean(DataSource.class)
    public DataSource dataSource(){
        DruidDataSource druidDataSource = new DruidDataSource();
//        将Druid在此配置各种属性非常麻烦且已经写死，解决办法：将配置文件中的DataSource与Druid绑定
...
发现Druid监控并没有记录，原因是没有配置StatFilter
...
配置类中加入`druidDataSource.setFilters("stat");`

可以看到，已经可以记录了

![image-20210226163139782](image/image-20210226163139782.png)
...
![image-20210227094224845](image/image-20210227094224845.png)
...
编写mybatis-config.xml和Mapper

参考https://mybatis.org/mybatis-3/zh/index.html
...
<mapper namespace="com.example.demo.dao.AccountMapper">
    <select id="selectById" resultType="com.example.demo.bean.Account">
    select * from account where id = #{id}
  </select>
</mapper>
```

编写Controller和service，跟以前一样
...
```
 ...
 可以不写全局；配置文件，所有全局配置文件的配置都放在configuration配置项中即可，如果都存在会报错

> Failed to instantiate [org.apache.ibatis.session.SqlSessionFactory]: Factory method 'sqlSessionFactory' threw exception; nested exception is java.lang.IllegalStateException: Property 'configuration' and 'configLocation' can not specified with together
...
public interface AccountMapper {
    ...
    ArrayList<Account> selectByMoney(double money);
}
```
...
public User selectByName(@RequestParam("name") String name){
    ...
    UserDao baseMapper = userService.getBaseMapper();
...
![image.png](image/1606745732785-17d1227a-75b9-4f00-a3f1-7fc4137b5113.png)
